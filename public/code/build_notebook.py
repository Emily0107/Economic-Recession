...
for col in indicators_1968:
    plt.plot(scaled_1968.index, scaled_1968[col], label=col, color=COLOR_MAP[col])
...
for col in indicators_1990:
    plt.plot(scaled_1990.index, scaled_1990[col], label=col, color=COLOR_MAP[col])
...
def plot_zoom_before_recessions(df, indicator_cols, filename, recession_col='Recession', months_before=24):
    ...
    for i, (start_date, end_date) in enumerate(periods):
        ...
        for col in indicator_cols:
            ax.plot(plot_df.index, plot_df[col], label=col, color=COLOR_MAP[col])
...
for col in traj_1968.columns:
    plt.plot(traj_1968.index, traj_1968[col], label=col, color=COLOR_MAP[col], linewidth=2)
...
indicators_1990 = ["SP500", "VIX", "Gold", "Oil"] # Note: SP500 added to financial for VIX correlation if needed, but original used VIX, Gold, Oil
...
for col in traj_1990.columns:
    plt.plot(traj_1990.index, traj_1990[col], label=col, color=COLOR_MAP[col], linewidth=2)
...
cells = [
    ...
    nbf.v4.new_code_cell(resampling_code),
    nbf.v4.new_code_cell(df_creation_code),
    nbf.v4.new_markdown_cell("## 2. Overall Normalized Trends (Line Graphs)"),
    nbf.v4.new_code_cell(graph_1968_code),
    nbf.v4.new_code_cell(graph_1990_code),
    nbf.v4.new_markdown_cell("## 3. Zoom-In View (24 months before recessions)"),
    nbf.v4.new_code_cell(zoom_func_code),
    nbf.v4.new_markdown_cell("## 4. Average Indicator Trajectory Before Recession"),
    nbf.v4.new_code_cell(traj_func_code),
    nbf.v4.new_code_cell(traj_1968_interact),
    nbf.v4.new_code_cell(traj_1968_static),
    nbf.v4.new_code_cell(traj_1990_interact),
    nbf.v4.new_code_cell(traj_1990_static),
    nbf.v4.new_markdown_cell("## 5. Individual Indicator Changes Before Recession"),
    nbf.v4.new_code_cell(indiv_change_code),
    nbf.v4.new_code_cell(indiv_change_1990_code),
    nbf.v4.new_markdown_cell("## 6. Correlation between Indicators"),
